...
function Orders() {
  ...
  const updateStatusHandler = async (e, orderId) => {
    try {
      const response = await axios.post(backendUrl + "/orders/status", {orderId, status: e.target.value}, {
        headers : {token}
      })
          ...
          >
            ...
            <div>
              {order.items.map((item, idx) => {
                const product = list.find((p) => p._id === item.productId); // Find product by ID
                const price = product ? product.price : "N/A"; // Get price if available
